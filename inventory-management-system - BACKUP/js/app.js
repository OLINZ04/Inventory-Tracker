...
async function loadInventory() {
    ...
    item.boxes = parseInt(item.boxes);
    item.pieces = parseInt(item.pieces);
});
...
function renderInventory(items) {
    ...
    if (items.length === 0) {
        ...
        // Inside your render function in app.js
if (items.length === 0) {
    tbody.innerHTML = `
        <tr>
    ...
    <tr>
        ...
        <td>
            <div class="actions-group">
                <button class="btn btn-secondary" onclick="openUpdateStockModal(${item.id})">Update</button>
...
async function confirmAddItem() {
    ...
    try {
        ...
        if (result.message.includes('success')) {
            result.item.id = parseInt(result.item.id);
...
function printInventory() {
    ...
    const categoryOrder = [
        "EASY|Syrup 2.5kg",
        "DRIZZLE 2.5kgx6|drizzle",
        "ESSENTIAL|easy e",
        "TOPPINGS|easy ts",
        "POWDER BASE|easy pb",
        "SOFT SERVE BASE|easy ss",
        "SIG. SYRUP|signature sy",
        "SIG. POWDER BASE|signature pb",
        "POWDER MIX|powder mix",
        "FRIES POWDER 100g|fries",
        "CHEESE SAUCE DIP|dip",
        "PREMIUM SAUCES|premium",
        "PLASTIC CUPS AND LIDS",
        "PLASTIC PRODUCTS|easy ts",
        "STYRO PRODUCTS|stp",
        "PAPER PRODUCTS|papr",
        "MICROWABABLES|micro",
        "EQUIPMENTS|greq",
        "CONES|cones",
        "CUP ORGANIZERS|cup org"
    ];
    ...
    .then(data => {
        ...
        // Sort by category order, then alphabetically by item name
        data.sort((a, b) => {

            const catAIndex = categoryOrder.indexOf(a.category);
            const catBIndex = categoryOrder.indexOf(b.category);

            // If category not found in list, send to bottom
            const safeA = catAIndex === -1 ? 999 : catAIndex;
            const safeB = catBIndex === -1 ? 999 : catBIndex;
            ...
            return a.item_name.toLowerCase()
...
function openUpdateStockModal(id) {
    const item = allItems.find(i => parseInt(i.id) === parseInt(id));
    if (!item) return;

    currentUpdateId = id;

    document.getElementById('updateStockItemName').textContent = item.item_name;
    document.getElementById('updateStockBoxes').value = item.boxes;
    document.getElementById('updateStockPieces').value = item.pieces;
...
async function confirmUpdateStock() {
    const boxes = parseInt(document.getElementById('updateStockBoxes').value) || 0;
    const pieces = parseInt(document.getElementById('updateStockPieces').value) || 0;

    try {
        const response = await fetch(`${API_BASE_URL}?action=update_stock`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                item_id: currentUpdateId,
                boxes: boxes,
                pieces: pieces
            })
        });
        ...
        if (result.message.includes('success')) {
            ...
            // Update local state
            const index = allItems.findIndex(i => i.id === currentUpdateId);
            if (index !== -1) {
                allItems[index].boxes = boxes;
                allItems[index].pieces = pieces;
                allItems[index].total_pieces =
                    (boxes * (allItems[index].pieces_per_box || 24)) + pieces;
            }
