...
switch($method) {
    ...
    case 'POST':
        ...
        elseif($request === 'update_stock') updateStock($db, $inputData);
...
function updateStock($db, $data){
    ...
    $boxes = isset($data->boxes) ? intval($data->boxes) : 0;
    $pieces = isset($data->pieces) ? intval($data->pieces) : 0;

    // Get pieces_per_box
    $stmt = $db->prepare("SELECT pieces_per_box FROM items WHERE id=:id");
    $stmt->bindParam(':id', $data->item_id, PDO::PARAM_INT);
    $stmt->execute();
    $item = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $total = ($boxes * intval($item['pieces_per_box'])) + $pieces;

    $update = $db->prepare("UPDATE items 
        SET boxes=:boxes, pieces=:pieces, total_pieces=:total 
        WHERE id=:id");
    ...
    echo $update->execute()
        ? json_encode(["message"=>"Stock updated successfully"])
        : json_encode(["message"=>"Failed to update stock"]);
}
